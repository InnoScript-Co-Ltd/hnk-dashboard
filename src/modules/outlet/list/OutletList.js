...
import { setPaginate } from "../outletSlice";
...
export const OutletList = () => {
    const { outlets, paginateParams } = useSelector((state) => state.outlet);
    const dispatch = useDispatch();

    const [total, setTotal] = useState(0);
    const [columnIds, setColumnIds] = useState("");
    const [sort, setSort] = useState(true);
    const outletColumns = useRef(outletPayload.columns);
    ...
    const loadingData = useCallback(async () => {
        const result = await outletService.index(dispatch, paginateParams);
                ...
                <TableContainer sx={{ maxHeight: 540 }}>
                    <Table sx={{ minWidth: 500 }}>
                        <TableHead>
                            ...
                            <TableRow>
                                {outletColumns.current.map((column) => (
                                    <TableCell
                                        key={column.id}
                                        align={column.align}
